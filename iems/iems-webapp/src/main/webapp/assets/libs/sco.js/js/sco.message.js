...
;(function($, undefined) {
	...
	$[pluginName] = function(message, type, callback) {
		...
		if (type === undefined || type == $[pluginName].TYPE_ERROR) {
			$selector
				.removeClass($[pluginName].options.okClass)
				.removeClass($[pluginName].options.warnClass)
				.addClass($[pluginName].options.errClass);
		} else if (type == $[pluginName].TYPE_WARN) {
			$selector
				.removeClass($[pluginName].options.okClass)
				.removeClass($[pluginName].options.errClass)
				.addClass($[pluginName].options.warnClass);
