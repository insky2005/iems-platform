...
;(function($, undefined) {
	...
	function Confirm(options) {
		...
		if (!$modal.length) {
			$modal = $('<div class="modal ' + this.options.cssclass + '" id="' + this.options.target.substr(1) + '">'+
					'<div class="modal-dialog">'+
					'<div class="modal-content">'+
					'<div class="modal-body inner"/>'+
					'<div class="modal-footer"><a class="btn btn-default cancel" href="#" data-dismiss="modal">'+this.options.cancel+'</a> <a href="#" class="btn btn-danger" data-action="1">'+this.options.yes+'</a></div>'+
					'</div>'+
					'</div>'+
		...
		if (typeof this.options.action == 'function') {
			...
			$modal.find('[data-action]').attr('href', '#').on('click.' + pluginName, function(e) {
				e.preventDefault();
				self.options.action.call(self, self.options);
				self.close();
			});
